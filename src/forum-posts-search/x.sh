...
for ((i=$BEGIN; i<=1795; ++i)); do
	...
	sleep 0.5
	if [ -z "$X_USE_PROXY" ]; then
		wget --load-cookies=cookies.txt "http://174.127.195.163/bbs/forum-${FORUM}-$i.html" -O $i
	else
		curl --socks5 127.0.0.1:23456 --cookie cookie.jar "http://174.127.195.163/bbs/forum-${FORUM}-$i.html" -o $i
	fi
